...
function Home() {
  ...
  useEffect(() => {
    const fetchOffersAndProducts = async () => {
      ...
      try {
        // Fetch offers
        const { data: offersResponse } = await axios.get("https://pilse-and-pills.el.r.appspot.com/api/offers");
        ...
        const productRequests = uniqueProductIds.map(id =>
          axios.get(`https://pilse-and-pills.el.r.appspot.com/api/products/${id}`)
            .catch(error => {
  ...
  const addToCart = async (productId) => {
    try {
      ...
      const response = await axios.post(
        'https://pilse-and-pills.el.r.appspot.com/api/cart/add',
