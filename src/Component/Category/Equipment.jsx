...
function Equipment() {
  ...
  const specificCategoryID = "666b2f95c66fdd3ebd54d892";

  const filteredData = data.filter((product) => {
    const matches =
      product.category && product.category._id === specificCategoryID;
    console.log(
      `Product ID: ${product._id}, Category: ${
        product.category ? product.category._id : "N/A"
      }, Matches: ${matches}`
    ); // Debugging: log each product's category and if it matches
    return matches;
  });
